    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.audios.update", [$audio->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('audio_url') ? 'is-invalid' : '' }}" type="text" name="audio_url" id="audio_url" value="{{ old('audio_url', $audio->audio_url) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('completed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="completed" id="completed" value="1" {{ $audio->completed || old('completed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('token') ? 'is-invalid' : '' }}" type="number" name="token" id="token" value="{{ old('token', $audio->token) }}" step="1">
                @if($errors->has('token'))
                    <div class="invalid-feedback">
                        {{ $errors->first('token') }}
                    </div>
