                ...
                <tbody>
                    @foreach($audios as $key => $audio)
                        <tr data-entry-id="{{ $audio->id }}">
                            ...
                            <td>
                                @can('audio_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.audios.show', $audio->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('audio_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.audios.edit', $audio->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('audio_delete')
                                    <form action="{{ route('admin.audios.destroy', $audio->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
